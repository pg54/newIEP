<template>
    <div>
        ...
        <el-dialog title="修改分组名称" :visible="dialogModifyVisible" :close-on-click-modal="false" :show-close="false">
            <el-form :model="form">
        ...
        <el-dialog title="新增分组" :visible="dialogFormVisible" :close-on-click-modal="false" :show-close="false">
            <el-form :model="form">
        ...
        <el-dialog title="移动学生" :visible="dialogMoveTableVisible" :close-on-click-modal="false" :show-close="false">
        ...
        <el-dialog title="新增学生" :visible="isShowAddUserDialogFlag" size="small" :close-on-click-modal="false" :show-close="false">

            <el-form label-width="90px" :model="userInfo" style="padding-left:10px;margin:0px" :rules="rules" ref="ruleForm">
                <el-row :gutter="24">
                ...
                <el-row :gutter="24">
                    <el-col :span="12" class="content">
                        <el-form-item label="出生日期:" prop="BirthDate">
                            <el-date-picker type="date" placeholder="选择日期" v-model="userInfo.BirthDate" :picker-options="pickerOptions0" :editable="false" style="width:100%"></el-date-picker>
                        </el-form-item>
                    </el-col>
                ...
                <el-row :gutter="24">
                    <el-col :span="12" class="content">
                        <el-form-item label="诊断时间:">
                            <el-date-picker type="date" placeholder="选择日期" v-model="userInfo.DiagnosisDate" :editable="false" :picker-options="pickerOptions0" style="width:100%"></el-date-picker>
                        </el-form-item>
                    </el-col>
                </el-row>
            ...
            <div style="float:right;margin-bottom:10px;margin-right:25px">
                ...
                <el-button type="primary" @click="submitUserInfo('ruleForm')">确认</el-button>
            </div>
    ...
    data() {
        return {
            ...
            rules: {
                ...
                IDCard: [{ required: true, message: '请输入患者身份证信息', trigger: 'blur' }, { min: 15, max: 18, message: '请填写正确的身份证号码', trigger: 'blur' }],
                BirthDate: [{ type: 'date', required: true, message: '请填写患者出生日期', trigger: 'blur' }],
                Sex: [{ required: true, type: 'number', message: '请选择患者性别', trigger: 'blur' }]
            }
        };
    },
    methods: {
        ...
        submitUserInfo(formName) {
            this.$refs[formName].validate((valid => {
                if (valid) {
                    let checkResult = this.checkUserInfo(this.userInfo);
                    if (checkResult) {
                        let _this = this;
        ...
        checkUserInfo(userInfo) {
            userInfo.Name = userInfo.Name.split(" ").join("");
            if (userInfo.Name === '') {
                this.$message.warning('请填写患者名称')
                return false;
            }

            if (userInfo.Sex === '') {
                this.$message.warning('请填写患者性别')
                return false;
            }

            if (userInfo.BirthDate === '') {
                this.$message.warning('请填写患者出生日期')
                return false;
            }

            let idReg = /^(^[1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}$)|(^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])((\d{4})|\d{3}[Xx])$)$/;
            let idStatus = idReg.exec(userInfo.IDCard);
            if (!idStatus) {
                this.$message.warning('请填写患者的正确的身份证号');
                return false;
            }
            return true;
        },
